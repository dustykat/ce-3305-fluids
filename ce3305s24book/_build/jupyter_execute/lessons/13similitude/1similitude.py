...
# :::
# 
# Similitude is a tool used to scale laboratory results to full-scale situations.  Many practical problems have unusual enough geometry that studies on a replica at small scale are used to predict flow patterns, pressure variation, frictional losses.
# 
# The scaling is usually done to preserve ratios of dimensionless groups identified in advance by dimensional analysis.
...
# 
# Such data are found in handbooks, journals, and other authorative sources.
# 
# Some examples are friction loss coefficients for pipes, valves, and other fittings; drag coefficients; fluid properties. 
# 
# ### Similitude 
# 
# Similitude relationships are used to scale laboratory results to full-scale situations.  When such tests are performed:
# 
# - Replica == Model
# - Full Size == Prototype
# 
# ```{note}
# Physical modeling is common in fluid mechanics studies even today (circa 2022).  Scale models are used to study and estimate behavior at full scale (aka "prototype"), although CFD methods are steadily replacing physical modeling as a primary tool.
# ```
